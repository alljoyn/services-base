...
{
    //printf("TranslateToOBAuthType authNum=%d theCiphers=%d\n", authNum, theCiphers);
    switch (authNum) {
    case 0:
        return OPEN;
    ...
    for (int i = 0; i < length; ++i) {
        ...
        json_object_object_foreach(scanrec, key, val) {
            if (!strcmp(key, "ssid")) {
                ...
                //printf("ParseScanInfo - found ssid=%s\n", ssid);
                ...
                if (m_ScanArray[current].authType != WEP) {
                    ...
                    //printf("ssid=%s current=%d wpa=%d group_ciphers=%s authType=%d\n\n", m_ScanArray[current].SSID.c_str(), current, wpa, GCiphers, m_ScanArray[current].authType);
                }
            }
        }
        ...
        if (it == m_ScanList.end()) {
            //printf("ssid=%s current=%d authType=%d\n\n", m_ScanArray[current].SSID.c_str(), current, m_ScanArray[current].authType);
...
{
    printf("entered execute_configure\n");
#ifdef _OPEN_WRT_
    char cmd[CMD_SIZE] = { 0 };
...
{
    printf("entered CancelAdvertise\n");
    ...
    if (m_BusAttachment->IsConnected() && m_BusAttachment->GetUniqueName().size() > 0) {
        ...
        printf("CancelAdvertiseName %s =%d\n", m_BusAttachment->GetUniqueName().c_str(), status);
    }
}

void OnboardingControllerImpl::AdvertiseAndAnnounce()
{
    printf("entered AdvertiseAndAnnounce\n");
    m_BusAttachment->EnableConcurrentCallbacks();
    if (m_BusAttachment->IsConnected() && m_BusAttachment->GetUniqueName().size() > 0) {
