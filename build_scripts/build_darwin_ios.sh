...
for SERVICE in ${SERVICES[@]}; do 
	...
	do
		...
		# compile sample app
		( xcodebuild -project $APP_DIR/$SERVICE_NAME.xcodeproj CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO -sdk $SDK -configuration $VARIANT ALLJOYN_SDK_ROOT=$WORKSPACE/core/alljoyn ALLSEEN_BASE_SERVICES_ROOT=$WORKSPACE/services/base ENABLE_BITCODE=NO clean build )
		if [ $? != 0 ]
		then 
