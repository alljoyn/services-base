...
- (void)UpdateCallViewInformation
{
	...
	if (self.sessionId == 0 || self.sessionId == -1) {
        ...
        if (qStatus != ER_OK) {
            ...
            NSLog(@"[%@] [%@] objectDescriptions:  %@", @"DEBUG", [[self class] description], [AJSCAboutDataConverter objectDescriptionsDictionaryToString:objDesc]);
            ...
            self.txtViewBusObjectDesc.text = [AJSCAboutDataConverter objectDescriptionsDictionaryToString:objDesc];
        }
    }
}
