...
/**
 * This is the base class for all the classes implementing {@link Timer} interface.
 * Extend this class to handle AllJoyn communication with this {@link Timer}.
 * Most of the {@link Timer} related calls delegated to the {@link org.allseen.timeservice.server.Timer}
 * object.
 */
...
public abstract class BaseTimerBusObj /*implements Timer*/ {
    private static final String TAG = "ajts" + BaseTimerBusObj.class.getSimpleName();

    /**
     * Timer to be notified with the {@link org.allseen.timeservice.server.Timer} related calls
     */
     ...
     * @param timer  {@link org.allseen.timeservice.server.Timer} delegate
     * @param parentObj Parent object path if the object was created via {@link TimerFactory}
     * @param needAnnounce If the object was created via {@link TimerFactory} no need to Announce
     ...
     * @throws TimeServiceException Is thrown if failed to initialized the object
     */
    void init(org.allseen.timeservice.server.Timer timer, String parentObj, boolean needAnnounce, String[] notAnnounced,
                    String description, String language, Translator translator) throws TimeServiceException {
        ...
        tsServer.registerBusObject((BusObject) this, objectPath, description, language, translator);

        //No need to send announcement if this Timer object was created with TimerFactory
        if ( needAnnounce ) {

            announcedInterfaces = tsServer.getAnnouncedInterfaces(this, notAnnounced, true);
            if ( announcedInterfaces.length != 0 ) {

                Log.i(TAG, "Announcable interfaces: '" + Arrays.toString(announcedInterfaces) + "'");
                AboutServiceImpl.getInstance().addObjectDescription(objectPath, announcedInterfaces);
            }
        }
    }
    ...
    void sendTimerEvent() throws TimeServiceException {
        ...
        SignalEmitter emitter = new SignalEmitter((BusObject) this, GlobalBroadcast.Off);
    ...
    void sendRunStateChanged(boolean isRunning) throws TimeServiceException {
        ...
        SignalEmitter emitter = new SignalEmitter((BusObject) this, sid, GlobalBroadcast.Off);
    ...
    void release() {
        ...
        if ( about.isServerRunning() && announcedInterfaces != null &&
                announcedInterfaces.length > 0 ) {

            Log.d(TAG, "Remove object description of: '" + Arrays.toString(announcedInterfaces) + "'");
            about.removeObjectDescription(objectPath, announcedInterfaces);
     ...
     * @throws TimeServiceException if {@link org.allseen.timeservice.server.Timer} hasn't been created yet
     */
